...
import net.darkz.featherapi.extension.DrawContextExtension;
...
public class BackgroundRenderer {
	...
	public static void drawWidgetBackground(GuiGraphicsExtractor context, int x, int y, int width, int height, boolean enabled, boolean hovered) {
		drawWidgetBackground(context, x, y, width, height, enabled, hovered, false);
	}

	public static void drawWidgetBackground(GuiGraphicsExtractor context, int x, int y, int width, int height, boolean enabled, boolean hovered, boolean rotate) {
		if (!enabled) {
			DrawUtils.drawGuiTexture(context, FeatherAPI.spriteId("transparency/widget/disabled_widget"), x, y, width, height/*? if <1.21 {*//*, 300, 22, 2 *//*?}*/);
			return;
		}
		context.push();
		...
		if (hovered) {
			DrawUtils.drawGuiTexture(context, FeatherAPI.spriteId("transparency/widget/hovered_widget"), 0, 0, width, height/*? if <1.21 {*//*, 300, 22, 2 *//*?}*/);
		} else {
			DrawUtils.drawGuiTexture(context, FeatherAPI.spriteId("transparency/widget/widget"), 0, 0, width, height/*? if <1.21 {*//*, 300, 22, 2 *//*?}*/);
		}
		context.pop();
	}
