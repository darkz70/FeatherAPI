package net.darkz.featherapi.utils;
...
public final class CodecUtils {
	...
	public static <A> Codec<A> recursive(String name, Function<Codec<A>, Codec<A>> wrapped) {
		...
		//?} else {
		/*return new net.darkz.featherapi.utils.codec.RecursiveCodec<>(name, wrapped);
		 *///?}
	}

	public static <A> A parseNewInstanceHacky(Codec<A> codec) {
		try {
			return codec.decode(JsonOps.INSTANCE, /*? <=1.17.1 {*//*new JsonParser().parse("{}")*//*?} else {*/JsonParser.parseString("{}")/*?}*/)/*? if >=1.20.5 {*/.getOrThrow()/*?} else {*//*.getOrThrow(false, FeatherAPIClient.LOGGER::error)*//*?}*/.getFirst();
		} catch (Exception e) {
			throw new IllegalArgumentException("Failed to create new config instance for the %s mod. Usually this happens when there is option with no default value, please report this to the mod author.".formatted(FeatherAPI.MOD_NAME), e);
		}
	}
	...
	public static <T> void decode(Codec<T> codec, JsonElement o, Consumer<T> consumer) {
		try {
			T value = codec.decode(JsonOps.INSTANCE, o)/*? if >=1.20.5 {*/.getOrThrow()/*?} else {*//*.getOrThrow(false, FeatherAPIClient.LOGGER::error)*//*?}*/.getFirst();
			consumer.accept(value);
		} catch (Exception e) {
			FeatherAPIClient.LOGGER.warn("Failed to decode JsonElement:", e);
		}
	}
	...
	public static <T> T decode(String id, Codec<T> codec, JsonObject o) {
		if (o.has(id)) {
			try {
				return codec.decode(JsonOps.INSTANCE, o.get(id))/*? if >=1.20.5 {*/.getOrThrow()/*?} else {*//*.getOrThrow(false, FeatherAPIClient.LOGGER::error)*//*?}*/.getFirst();
			} catch (Exception e) {
				FeatherAPIClient.LOGGER.warn("Failed to decode \"%s\" from JsonObject:".formatted(id), e);
			}
		}
		return null;
	}
